...
engine.say("Object Recognizer Launched")
engine.runAndWait()

detectedObjects = []
##########Voice Output######

baseFolder = '/home/shubham/co_vision/yolo_object_detection/'

# load yolo
weights_file = os.path.join(baseFolder, 'yolov4-tiny.weights')
cfg_file = os.path.join(baseFolder, 'yolov4-tiny.cfg')
net = cv.dnn.readNet(weights_file, cfg_file)
classes = []
with open(os.path.join(baseFolder, 'coco.names'), 'r') as f:
...
while True:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            cv.putText(frame, (label), (x, y + 20), my_font, 2, color, 3)
